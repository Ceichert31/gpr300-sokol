...
void main()
{
  ...
  vec3 tangent = normalize(vec3(in_instancedMatrix * vec4(in_tangent, 0.0)));
  vec3 normal = normalize(vec3(in_instancedMatrix * vec4(in_normal, 0.0)));
  ...
  gl_Position = view_proj * in_instancedMatrix * vec4(in_position, 1.0);
}